...
import oleborn.rps_duel.bot.Bot;
...
public class OutputsMethods extends Bot {
    ...
    public void outputMessage(Long id, String text, String typeParseMode) {
        SendMessage ms = SendMessage.builder()
                .chatId(id)
                .parseMode(typeParseMode)
                .text(text)
                .build();
        try {
            sendApiMethod(ms);
    ...
    public void deleteMessageInChat(long chatId, int messageId) {
        try {
